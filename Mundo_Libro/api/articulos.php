...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    $input = $_POST;		
    $sql = "INSERT INTO libros (title, ranking) VALUES (:title, :ranking)";		  
...
if ($_SERVER['REQUEST_METHOD'] == 'PUT'){
    ...
    $sql = "UPDATE libros SET $campos WHERE id='$id'";
    $resultado = $conexion->prepare($sql);
    bindAllValues($resultado, $input);
    $resultado->execute();
    header("HTTP/1.1 200 OK");
...
if ($_SERVER['REQUEST_METHOD'] == 'DELETE'){
  $id = $_GET['id'];
  $resultado = $conexion->prepare("DELETE FROM libros where id=:id");
